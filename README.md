...
The first Milestone 3 slices are present in `gear-pep`: the sidecar now serves the ability-facing loopback API on `127.0.0.1:9191` only, with `GET /v1/healthz`, `POST /v1/extract`, and `POST /v1/effects`. Requests are validated against the active governed action. Effect requests now build the fixed ten-field policy input from trusted PEP state, call `gear-policy`, and enforce deny/escalate/authorise without executing raw effects yet.
